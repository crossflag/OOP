            ...
            public void actionPerformed(ActionEvent e) {
                ...
                Runnable myRunnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        AtomicInteger countWrong = new AtomicInteger(0);

                        log.info("Number of records: " + HANDLER.getTweetBuffer().size());
                        int numThreads = HANDLER.getTweetBuffer().size() / 50 + 1;
                        if (numThreads > 10) {
                            numThreads = 10;
                        }
                        log.info("Number of threads: " + numThreads);

                        BlockingQueue<Runnable> workQueue = new ArrayBlockingQueue<Runnable>(numThreads);
                        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(10, 20, 40, TimeUnit.SECONDS, workQueue);

                        for (int i = 1; i <= numThreads; i++) {
                            int size = HANDLER.getTweetBuffer().size()/numThreads;
                            int startIndex = size * (i-1);
                            int endIndex = size * i;
                            if (i == numThreads) {
                                endIndex = HANDLER.getTweetBuffer().size();
                            }
                            List<AbstractTweet> tempList = HANDLER.getTweetBuffer().subList(startIndex, endIndex);
                            log.info("Thread " + i + " on index " + startIndex + " to " + (endIndex-1) + ". Size: " + tempList.size());
                            Thread subThread = new Thread() {
                                public void run() {
                                    for (AbstractTweet t : tempList) {
                                        t.setPredictedPolarity(HANDLER.classifyTweet(t));
                                        if (t.getPredictedPolarity() == t.getTarget()) {
                                            countCorrect.getAndIncrement();
                                        } else {
                                            countWrong.getAndIncrement();
                                        }
                                    }
                                }
                            };
                            threadPoolExecutor.execute(subThread);
                        }
                        ...
                        outputArea.append("Correct classified tweets: " + countCorrect + "\n");
                        outputArea.append("Incorrect classified tweets: " + countWrong + "\n");
                        double correctRate = (double)countCorrect.intValue() / ((double)countWrong.intValue() + (double)countCorrect.intValue()) * 100;
